...
use omnity_indexer_sync::{
    ...
    get_wallet_wasm_from_env, universal::payload, with_universal_canister, with_wallet_canister,
};
use serde::Serialize;
use std::{
    borrow::Cow,
...
mod sign_send {
    ...
    use omnity_indexer_sync::{universal::payload, with_universal_canister};
    use std::{thread, time};
...
mod identity {
    ...
    use omnity_indexer_sync::{universal::payload, with_universal_canister_as};
    use ring::{
...
mod omnity_hub {
    ...
    fn test_sync_chains() {
        ...
        let db = runtime.block_on(async { Database::new().await });
        sync_chains(db.get_connection());
        let chains =
            runtime.block_on(async { Query::get_all_chains(db.get_connection()).await.unwrap() });
    ...
    fn test_sync_tokens() {
        ...
        let runtime = tokio::runtime::Runtime::new().expect("Could not create tokio runtime.");
        let db = runtime.block_on(async { Database::new().await });
        sync_tokens(db.get_connection());
        let tokens =
            runtime.block_on(async { Query::get_all_tokens(db.get_connection()).await.unwrap() });
    ...
    fn test_sync_tickets() {
        ...
        let runtime = tokio::runtime::Runtime::new().expect("Could not create tokio runtime.");
        let db = runtime.block_on(async { Database::new().await });
        sync_tickets(db.get_connection());
        let tickets =
            runtime.block_on(async { Query::get_all_tickets(db.get_connection()).await.unwrap() });
